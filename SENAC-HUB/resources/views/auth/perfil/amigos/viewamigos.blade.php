                    ...
                    <div class="profile">
                        <div class="profile-header">
                            ...
                            <ul class="profile-header-tab nav nav-tabs">
                                <li class="nav-item"><a href="{{ route('profile.show', ['userId' => $user->id]) }}" target="_blank">POSTS</a></li>
                    ...
                    <div class="profile-content">
                        ...
                        <div class="tab-content p-0">
                            ...
                            <div style="color: white;">
                                <h1 style="color: white;">Lista de Amigos</h1>
                                <ul>
                                @if($user->id === auth()->user()->id)
    @foreach ($user->friends as $friend)
        <li>{{ $friend->name }}</li>
        <div>
            <a href="{{ route('conversa.iniciar', ['amigoId' => $friend->id]) }}" class="btn btn-primary">Iniciar Conversa</a>
        </div>
    @endforeach
@endif
                                </ul>
                                <hr>
                                @php
                                // Busque as solicitações de amizade pendentes recebidas pelo usuário que está no perfil URL
                                $userPendingRequests = $user->receivedFriendRequests;
                                @endphp
                                ...
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        {{ $request->sender->name }} solicitou amizade
                                        <!-- Mostra os botões de aceitar ou recusar -->
                                        <form action="{{ route('accept-friend-request', $request->id) }}" method="POST">
                                            @csrf
                                            <button type="submit">Aceitar</button>
                                        </form>
                                        <form action="{{ route('decline-friend-request', $request->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        @endif
                                        @else
                                        <!-- Se a solicitação foi aceita, mostra a mensagem -->
                                        {{ $request->sender->name }} solicitação de amizade foi
                                        {{ $request->accepted ? 'aceita' : 'pendente' }}
                                        @endif
                                    </li>
                                    @endforeach
                                </ul>
                                @else
                                <p style="color: white;">Nenhuma solicitação de amizade pendente.</p>
                                @endif
                                ...
                                <ul>
                                    <li>
                                        ...
                                        <form id="sendFriendRequestForm{{ $user->id }}" action="{{ route('send-friend-request', $user->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" onclick="sendFriendRequest(event, {{ $user->id }})">Enviar
                                                Solicitação</button>
                                        </form>
                                    </li>
                                </ul>
